...
# Original implementation from https://github.com/KindXiaoming/pykan/blob/master/kan/KANLayer.py
# Chebvy spline with KAN to PINN
class ChebyKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.degree >= 1:
            cheby2[:, :, 1] = x  # First-order polynomial: U_1(x) = 2x
        if self.degree >= 2:
            cheby2[:, :, 2] = 2 * x**2 - 1  # Second-order polynomial: U_2(x) = 4x^2 - 1
        if self.degree >= 3:
            cheby2[:, :, 3] = 4 * x**3 - 4 * x  # Third-order polynomial: U_3(x) = 8x^3 - 4x
        if self.degree >= 4:
            cheby2[:, :, 4] = 8 * x**4 - 8 * x**2 + 1  # Fourth-order polynomial: U_4(x) = 16x^4 - 8x^2 + 1
...
class ChebyPINN(nn.Module):
    ...
    def __init__(self, network, degree = 3):
        ...
        Args:
            network (list[int]): List defining the number of nodes in each layer.
        Example:
            >>> network = [2, 50, 50, 1]  # Example architecture
            >>> degree = 3  # Degree of Chebyshev polynomials
        """
        super(ChebyPINN, self).__init__()
